...
@dataclass
class Sample:
    row: InitVar[dict[str, str]]
    tach: float = field(init=False)
    engine: float = field(init=False)

    def __post_init__(self, row: dict[str, str]) -> None:
        self.tach = float(row["Tach"])
        self.engine = float(row["Engine"])
...
class Reader:
    ...
    def read(self) -> None:
        with self.source.open() as input:
            ...
            self.samples = [Sample(row) for row in reader]
