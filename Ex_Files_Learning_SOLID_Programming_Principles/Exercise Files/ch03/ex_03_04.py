...
from ex03_config import Configuration
...
def main(source: Path) -> None:
    configs: Iterator[ReaderConfig] = (
        config_detail
        for name, config_detail in Configuration.__dict__.items()
        if not name.startswith("_")
    )
    config_map: dict[str, ReaderConfig] = {
        ext: detail for detail in configs for ext in detail.extensions
    }
    try:
        config = config_map[source.suffix]
    except KeyError:
