...
class ConfigBuilder:
    def __init__(self, config_path: Path = Path("ex03.toml")) -> None:
        with config_path.open() as config_file:
            self.config = toml.load(config_file)

    def make_reader(self, suffix: str, **extra: Any) -> Reader:
        section = self.config["FORMAT"]
        try:
            subsection = section[suffix]
            row_generator = eval(subsection["row_generator"])
            sample_builder = eval(subsection["sample_builder"])
        except (KeyError, NameError) as ex:
            raise ValueError(f"Can't process {suffix}: {ex}")
        return Reader(row_generator, sample_builder)

    def make_analysis(self, samples: SampleList) -> Correlation:
        class_: Type[Correlation] = eval(self.config["APP"]["analysis_class"])
        return class_(samples)


def main(source: Path) -> None:
    config = ConfigBuilder()
    reader = config.make_reader(source.suffix)
    reader.read(source)

    analysis = config.make_analysis(reader.samples)
